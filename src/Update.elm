...
doSize app size =
  let
    validatedSize = (S.toInt size) `R.andThen` validateSize
  in
...
doKomi app komi =
  let
    validated = (S.toFloat komi) `R.andThen` validateKomi
  in
    case validated of
      Ok komi_ ->
        app
...
doHandicap app handicap =
  let
    validated = (S.toInt handicap) `R.andThen` validateHandicap
  in
    case validated of
      Ok handicap_ ->
        app
...
update msg app =
  case msg of
    Msg.Start -> doStart app
    Msg.Size size -> doSize app size
    Msg.Komi komi -> doKomi app komi
    Msg.Handicap handicap -> doHandicap app handicap
    Msg.Game msg_ -> updateGame (G.update msg_) app
