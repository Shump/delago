...
type Digits = Digits Int (List Int)

toString : String -> Digits -> String
toString alpha digits =
  let
    alpha' = String.toList alpha
    base = String.length alpha
    (Digits _ digs) = toBase base digits
    chars = List.map (\d -> getAt d alpha') digs
  in
...
toInt (Digits base digs) =
  let
    acc dig (pow, val) = (pow + 1, val + (base ^ pow) * dig)
  in
...
fromString alphabet base string =
  let
    alpha = String.toList alphabet
    chars = String.toList string
    f c = elemIndex c alpha
    encoded = traverse f chars
  in
    case encoded of
      Nothing -> Nothing
      Just digits -> Just <| Digits base digits

fromInt : Int -> Digits
fromInt num = toBase' 10 num
